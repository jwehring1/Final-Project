...
<tags:template>
	...
	<jsp:body>
		...
		<c:forEach items="${ SearchPplByAddrView }" var = "p">
		<tr class = viewth>
			...
			<td class = viewtd>${p.addr.street}</td>
			<td class = viewtd>${p.addr.city}</td>
			<td class = viewtd>${p.addr.state}</td>
			<td class = viewtd>${p.addr.zipcode}</td>
			</tr>
		</c:forEach>
		</table>
		</p>
